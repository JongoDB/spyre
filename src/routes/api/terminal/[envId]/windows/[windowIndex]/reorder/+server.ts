...
import { ensureSession, swapWindow } from '$lib/server/tmux-controller';
...
export const POST: RequestHandler = async ({ params, request }) => {
  ...
  const sourceIndex = parseInt(params.windowIndex, 10);
  if (isNaN(sourceIndex)) {
    return json({ code: 'BAD_REQUEST', message: 'Invalid window index' }, { status: 400 });
  }

  const body = await request.json().catch(() => ({}));
  const targetIndex = typeof body.targetIndex === 'number' ? body.targetIndex : NaN;
  if (isNaN(targetIndex)) {
    return json({ code: 'BAD_REQUEST', message: 'targetIndex is required' }, { status: 400 });
  }

  try {
    const client = await getConnection(params.envId);
    await ensureSession(client);
    await swapWindow(client, 'spyre', sourceIndex, targetIndex);
    return json({ ok: true });
  } catch (err) {
