<script lang="ts">
	...
	// Resources
	let resourcePresetId = $state('');
	let cores = $state<number | ''>('');
	let memory = $state<number | ''>('');
	let swap = $state<number | ''>('');
	let disk = $state<number | ''>('');
	let storage = $state('');

	// Network
	let networkProfileId = $state('');
	let bridge = $state('');
	let ipMode = $state<'dhcp' | 'static'>('dhcp');
	let ipAddress = $state('');
	let gateway = $state('');
	let dns = $state('');
	let vlan = $state<number | ''>('');

	// Container settings
	let unprivileged = $state(true);
	let nesting = $state(true);
	let features = $state('');
	let startupOrder = $state<number | ''>('');
	let protection = $state(false);
	...
	async function handleSubmit(e: SubmitEvent) {
		...
		// Resources
		if (resourcePresetId) {
			body.resource_preset_id = resourcePresetId;
		}
		if (cores !== '') body.cores = Number(cores);
		if (memory !== '') body.memory = Number(memory);
		if (swap !== '') body.swap = Number(swap);
		if (disk !== '') body.disk = Number(disk);
		if (storage) body.storage = storage;

		// Network
		if (networkProfileId) {
			body.network_profile_id = networkProfileId;
		}
		...
		if (defaultUser) body.default_user = defaultUser;
		...
		// Custom script & tags
		if (customScript) body.custom_script = customScript;
		...
		try {
			...
			if (!res.ok) {
				...
				errorMessage = result.message ?? `Failed to create template (HTTP ${res.status}).`;
				return;
			}
...
<div class="form-page">
	<header class="page-header">
		...
		<h1>New Template</h1>
		<p class="subtitle">Define a reusable blueprint for provisioning environments.</p>
	</header>
